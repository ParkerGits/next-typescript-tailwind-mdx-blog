...
In this lesson, we use [CodeSandbox](https://codesandbox.io/) to quickly begin working on our React component.
...
In this lesson, we will set up a Git repo and deploy to Netlify.

We create a new directory, initialize Git with `git init`, create a package.json with `yarn init -y`, then create an `index.html` and a `netlify.toml`.
...
In this lesson, we'll create our first function. This function will generate our OpenGraph image using Playwright.
