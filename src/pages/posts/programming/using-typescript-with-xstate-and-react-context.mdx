...
The gif above is from a project that I am currently working on as part of an [egghead.io Portfolio Project Club](https://egghead.io/clubs/portfolio-project). You can follow the progress that I'm making on it [here](https://github.com/ParkerGits/RPG-Portfolio), but, essentially, I am planning to have my developer portfolio be presented as an RPG shop cutscene.
...
In order to provide the current state of the machine to both the `Player` component and the `DialogueBox` component, I set up the machine to be provided to my application via React Context:
